...
class MeasureStore:
    ...
    STORE_NAME = 'wiretap_measures.db'
    ...
    @staticmethod
    def copy_measures_db_if_not_exists(src_path=None, dest_path=None):
        if src_path is None:
            src_path = MeasureStore.PRE_STORE_PATH
        if dest_path is None:
            dest_path = MeasureStore.STORE_PATH
    ...
    @classmethod
    def __check_tuple(cls, data:Tuple[str, str, float, Union[str, datetime]]) -> Tuple[str, str, float, datetime]:
        ...
        elif len(data) >= 4 and not isinstance(data[3], datetime):

            # convert data[3] to datetime and replace in tuple
            data = data[0:3] + (_to_datetime(data[3]), )
    ...
    @classmethod
    def insert(cls, tup_or_device:Union[str, Tuple[str, str, float, Union[str, datetime]]], measure_name:str = None, measure_value:float = None, timestamp:Union[str, datetime] = None):
        ...
        if isinstance(tup_or_device, (tuple, list)):

            # check time in tuple and tup_or_device list to tuple
            entry = cls.__check_tuple(tup_or_device)

        else:


            if not(isinstance(tup_or_device, str)):

                exc_msg = _to_err_msg("in 'WiretapDatabase.insert', expected parameter 'tup_or_uuid' to be of type",
                    f"'tuple' or 'list', or a parsable integer, got '{type(tup_or_device)}' with value '{tup_or_device}'")
                
                raise TypeError(exc_msg) from None

            entry = (tup_or_device, measure_name, measure_value, _to_datetime(timestamp))
    ...
    @classmethod
    def insert_dto(cls, dto:MeasureObj):
        ...
        if not isinstance(dto, MeasureObj):
    ...
    @classmethod
    def __dto_to_tuple(cls, dto:MeasureObj) -> Tuple[str, str, float, datetime]:
        
        return dto


    @classmethod
    def insert_many(cls, data:Iterable[Tuple[str, str, float, Union[str, datetime]]]):
    ...
    @classmethod
    def insert_many_dto(cls, dto_list:Iterable[MeasureObj]):
    ...
    @classmethod
    def __insert(cls, db_cursor:sqlite3.Cursor, data:Tuple[str, str, float, datetime]):
        # insert or replace handles missing row
        db_cursor.execute(f'''
            INSERT OR REPLACE INTO {cls.STORE_SPOT}
            (device, measure_name, measure_value, timestamp)
            VALUES
            (?, ?, ?, ?)
        ''', data)
    ...
    @classmethod
    def get_with_daq(cls, daq:str, options:Dict[str, Union[str, int, datetime]] = None) -> Union[MeasureObj, None]:
        ...
        row = cls.__query_many(where = "device=?", params = (daq,), options = options)

        return row


    @classmethod
    def get_all(cls, options:Dict[str, Union[str, int, datetime]] = None) -> Union[List[MeasureObj], None]:
    ...
    @classmethod
    def __query_one(cls, **kwargs) -> Union[MeasureObj, None]:
    ...
    @classmethod
    def __query_many(cls, **kwargs) -> Union[List[MeasureObj], None]:
    ...
    @classmethod
    def __row_to_dto(cls, row:tuple) -> MeasureObj:

        return MeasureObj(*row)
    ...
    def __init(cls):
        ...
        with cls.__connect() as db_con:
            ...
            db_cursor.execute(f'''
                ...
                measure_value REAL,
                timestamp TIMESTAMP,
                CONSTRAINT comp_key PRIMARY KEY (device, measure_name))
            ''')
...
MeasureStore._MeasureStore__init()
